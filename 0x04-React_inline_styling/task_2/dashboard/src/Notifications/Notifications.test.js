...
describe('listNotifications is rendered correctly when non empty', () => {
  ...
  it('renders the correctly NotificationItem', () => {
    expect(wrapper.find(NotificationItem).first().html()).toBe('<li data-notification-type="default" class="defaultColor_1tsdo2i">New course available</li>');
  });
});
...
it('markAsRead works correctly', () => {
  ...
  inst.markAsRead(1);
  expect(logM).toBeCalledWith("Notification 1 has been marked as read");
  logM.mockRestore();
});
