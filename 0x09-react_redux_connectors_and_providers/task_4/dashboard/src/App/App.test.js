...
it('markNotificationAsRead works well', () => {
  const inst = shallow(<App />).instance();
  expect(inst.state.listNotifications.length).toBe(3);
  inst.markNotificationAsRead(3);
  expect(inst.state.listNotifications.length).toBe(2);
});
